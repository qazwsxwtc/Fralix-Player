...
//
//void CFFmpegDecoder::SeekNew(int64_t targetMs)
//{
//    if (!m_pFormatCtx) return;
//
//    std::cout << "[Decoder] Seeking to " << targetMs << " ms..." << std::endl;
//
//    // 【关键步骤 1】 唤醒所有因暂停而阻塞的线程
//    // 即使当前没有暂停，调用 notify_all 也是安全的
//    ResetPauseState();
//
//    // 【关键步骤 2】 标记正在 Seek，防止新数据进入队列造成混乱
//    // 注意：这里不需要复杂的锁，因为我们会清空队列
//    m_bStopThreads = true; // 临时停止线程循环
//
//    // 等待线程完全退出或进入安全状态？ 
//    // 更简单的做法：直接清空队列，线程会在下一次循环检测到队列空或 abort 状态
//
//    // 【关键步骤 3】 清空所有队列
//    m_videoQueue.clear();
//    m_audioQueue.clear();
//    m_videoShowQueue.clear(); // FrameQueue 也需要 clear
//    m_audioPlayQueue.clear();
//
//    // 【关键步骤 4】 刷新解码器，丢弃内部缓冲
...
//
//    // 【关键步骤 5】 执行 FFmpeg seek
//    int64_t targetPts = targetMs * 1000; // 转换为微秒
//    int ret = av_seek_frame(m_pFormatCtx, -1, targetPts, AVSEEK_FLAG_BACKWARD);
//
//    if (ret < 0) {
//        std::cerr << "[Decoder] Seek failed!" << std::endl;
//        m_bStopThreads = false; // 恢复线程运行
//        return;
//    }
//
//    // 【关键步骤 6】 重置状态并重新启动线程
//    m_bStopThreads = false;
//    m_bPaused = false; // 确保处于播放状态
//
//    // 如果之前的线程已经退出（因为 m_bStopThreads=true），需要重新启动它们
//    // 这取决于你的线程管理方式。如果你的线程是 while(!m_bStopThreads) 循环，
//    // 设置 m_bStopThreads=false 后，它们会自动继续运行（如果没退出的话）。
//
//    // 如果你的设计是 Seek 时线程不退出，只是清空队列，那么只需确保 m_bStopThreads 为 false 即可。
//    // 如果线程退出了，你需要重新 join 并 start 新线程。
//
//    // 假设你的线程模型是：m_bStopThreads 控制循环退出。
//    // 如果 Seek 导致线程退出，你需要在这里重新创建线程。
//    // 但通常更好的做法是：Seek 时不杀线程，只清空队列。
//
//    // 修正：上面的 m_bStopThreads = true 会导致线程退出。
//    // 更好的 Seek 实现是不杀线程，而是使用一个专门的 m_isSeeking 标志，或者仅仅依靠 clear() 和 flush。
//
//    // --- 优化后的 Seek 逻辑（不杀线程） ---
//
//    /*
//    1. ResetPauseState(); // 唤醒
//    2. m_videoPacketQueue.clear(); ... // 清空
//    3. avcodec_flush_buffers(...);
//    4. av_seek_frame(...);
//    5. m_bPaused = false; // 确保不暂停
//    */
//
//    // 由于你之前的代码中 m_bStopThreads 用于彻底停止，建议 Seek 时使用单独的标志或仅依赖队列清空。
//    // 这里假设我们采用“不杀线程”的策略：
//
//    // 1. 唤醒
//    ResetPauseState();
//
//    // 2. 清空队列 (clear 内部会加锁并通知)
//	m_videoQueue.clear();
//	m_audioQueue.clear();
//	m_videoShowQueue.clear(); // FrameQueue 也需要 clear
//	m_audioPlayQueue.clear();
...
//
//    // 4. Seek
//    ret = av_seek_frame(m_pFormatCtx, -1, targetPts, AVSEEK_FLAG_BACKWARD);
//
//    if (ret >= 0) {
//        std::cout << "[Decoder] Seek Success." << std::endl;
//    }
//    else {
//        std::cerr << "[Decoder] Seek Failed." << std::endl;
//    }
